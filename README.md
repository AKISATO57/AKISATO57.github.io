...
由於本專案是單文件架構，您不需要 npm install 或 npm run build。

### **1\. 下載**

將 index.html 下載到您的本地電腦。

### **2\. 運行**

雖然可以直接雙擊打開，但為了確保 ES Modules 和 fetch API 正常工作，建議使用簡易的本地服務器：

VS Code 用戶 (推薦):  
安裝 "Live Server" 插件，右鍵 index.html 選擇 "Open with Live Server"。  
Python 用戶:  
在目錄下運行終端機：  
\# Python 3.x  
